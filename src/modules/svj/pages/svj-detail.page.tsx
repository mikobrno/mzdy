...
export function SvjDetailPage() {
  ...
  const monthlyPayrollStatus = Array.from({ length: 12 }, (_, i) => {
    const month = i + 1
    const monthPayrolls = payrolls?.filter((p: any) => p.month === month) || []
    return {
      month,
            ...
            <Card>
              ...
              <CardContent className="space-y-2 text-sm text-gray-600">
                <div className="flex justify-between">
                  <span>Vytvořeno:</span>
                  <span>{formatDate((svj as any).createdAt || (svj as any).created_at)}</span>
                </div>
                <div className="flex justify-between">
                  <span>Aktualizováno:</span>
                  <span>{formatDate((svj as any).updatedAt || (svj as any).updated_at)}</span>
                </div>
                {(svj as any).registryData?.verificationDate && (
                  <div className="flex justify-between">
                    <span>Ověřeno z rejstříku:</span>
                    <span>{formatDate((svj as any).registryData.verificationDate)}</span>
                  </div>
                )}
              </CardContent>
            </Card>
