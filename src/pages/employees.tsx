          ...
          <CardContent className="pt-6">
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">{filteredEmployees.filter(e => e.executions.length > 0).length}</p>
              </div>
            </div>
          </CardContent>
        </Card>
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    {employee.isActive && (
                      <Button 
                        ...
                        size="sm"
                        onClick={() => {
                          const confirmed = window.confirm(`Opravdu chcete ukončit zaměstnance ${employee.firstName} ${employee.lastName}?`)
                          if (confirmed) {
                            terminateEmployeeMutation.mutate(employee.id)
                          } else {
                            warning('Akce zrušena')
                          }
                        }}
                        disabled={terminateEmployeeMutation.isPending}
                        aria-label={`Ukončit zaměstnance ${employee.firstName} ${employee.lastName}`}
                      >
